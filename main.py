...
# Almacenamiento en memoria (sin BD para simplificar la demo) 
alerts_cache: list[dict] = []
fires_cache:  list[dict] = []


# AEMET: obtención y parseo 
...
async def fetch_aemet_alerts() -> list[dict]:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for area in ["esp", "can"]:
            ...
            # 3: extraer XMLs del TAR y parsear
...
# Geometria oficial de Espana cargada desde GeoJSON publico al arrancar
# Incluye Peninsula, Baleares, Canarias, Ceuta y Melilla
_SPAIN_GEOM = None
...
async def load_spain_geometry():
    ...
    try:
        ...
        print(f"  Geometria de Espana cargada correctamente")
    except Exception as e:
        print(f"  Error cargando geometria: {e} — usando bbox de respaldo")
        from shapely.geometry import box
        _SPAIN_GEOM = unary_union([
            box(-9.3, 36.0, 3.3, 43.8),   # Peninsula aproximada
            box(1.1, 38.6, 4.4, 40.1),    # Baleares
            box(-18.2, 27.6, -13.3, 29.5), # Canarias
            box(-5.4, 35.85, -5.2, 35.95), # Ceuta
            box(-2.97, 35.26, -2.93, 35.30), # Melilla
        ])
...
async def fetch_firms_fires() -> list[dict]:
    if not settings.firms_map_key:
        return []
    url = f"https://firms.modaps.eosdis.nasa.gov/api/area/csv/{settings.firms_map_key}/VIIRS_SNPP_NRT/{SPAIN_BBOX}/1"
...
@app.get("/api/alerts")
def get_alerts():
    """Devuelve todos los avisos meteorológicos activos de AEMET."""
    return alerts_cache

@app.get("/api/fires")
def get_fires():
    """Devuelve todos los focos de incendio activos de NASA FIRMS."""
    return fires_cache
