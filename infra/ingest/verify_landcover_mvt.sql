...
SELECT 'pub.landcover_mvt_class_source' AS object_name, to_regclass('pub.landcover_mvt_class_source') IS NOT NULL AS exists;
...
SELECT
    ...
    count(*) FILTER (WHERE NOT ST_IsValid(geom, 0)) AS invalid_geom,
    count(*) FILTER (WHERE ST_IsEmpty(geom)) AS empty_geom
FROM pub.landcover_mvt_class_source;
