...
CREATE TABLE IF NOT EXISTS core.landcover_class (
    class_code text PRIMARY KEY CHECK (class_code IN ('1001', '121', '211', '242', '311', '312', '313', '321', '322', '323', '324')),
    class_label text NOT NULL,
    class_color text NOT NULL CHECK (class_color ~ '^#[0-9A-Fa-f]{6}$'),
    theme text NOT NULL CHECK (theme IN ('artificial', 'forest', 'grassland', 'shrubland', 'agriculture')),
    display_order smallint NOT NULL UNIQUE
);

ALTER TABLE core.landcover_class
    DROP CONSTRAINT IF EXISTS landcover_class_class_code_check;
ALTER TABLE core.landcover_class
    ADD CONSTRAINT landcover_class_class_code_check
    CHECK (class_code IN ('1001', '111', '112', '121', '211', '242', '311', '312', '313', '321', '322', '323', '324'));

ALTER TABLE core.landcover_class
    DROP CONSTRAINT IF EXISTS landcover_class_theme_check;
ALTER TABLE core.landcover_class
    ADD CONSTRAINT landcover_class_theme_check
    CHECK (theme IN ('artificial', 'forest', 'grassland', 'shrubland', 'agriculture'));

UPDATE core.landcover_class
SET display_order = display_order + 900
WHERE class_code IN ('111', '112');

INSERT INTO core.landcover_class (class_code, class_label, class_color, theme, display_order)
VALUES
    ('1001', 'Tejido urbano', '#e6004d', 'artificial', 100),
    ('121', 'Zonas industriales o comerciales', '#cc4df2', 'artificial', 120),
...
ALTER TABLE core.landcover_polygon
    DROP CONSTRAINT IF EXISTS landcover_polygon_theme_check;
ALTER TABLE core.landcover_polygon
    ADD CONSTRAINT landcover_polygon_theme_check
    CHECK (theme IN ('artificial', 'forest', 'grassland', 'shrubland', 'agriculture'));

UPDATE core.landcover_polygon
SET class_code = '1001',
    class_label = 'Tejido urbano',
    class_color = '#e6004d',
    theme = 'artificial'
WHERE class_code IN ('111', '112');

DELETE FROM core.landcover_class
WHERE class_code IN ('111', '112');
...
ALTER TABLE core.landcover_class
    ADD CONSTRAINT landcover_class_class_code_check
    CHECK (class_code IN ('1001', '121', '211', '242', '311', '312', '313', '321', '322', '323', '324'));
...
CREATE INDEX IF NOT EXISTS landcover_polygon_canonicalized_at_idx
    ON core.landcover_polygon (canonicalized_at DESC);
...
COMMENT ON COLUMN core.landcover_polygon.class_code IS 'Codigo canonico de publicacion; puede agrupar varios CODE_18 del origen CORINE';
...
COMMENT ON COLUMN core.landcover_polygon.theme IS 'Tema homogeneo de explotacion: artificial, forest, grassland, shrubland o agriculture';
